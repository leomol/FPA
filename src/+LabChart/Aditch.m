% Aditch - Loads LabChart's adicht file.
% 
% information = LabChart.Aditch.information(filename)
% [time, data] = LabChart.Aditch.load(filename)
...
classdef Aditch
    methods (Static)
        function [time, data] = load(filename)
        ...
        function [information, relativeComments, absoluteComments] = information(filename)
            filePointer = getFilePointer(filename);
            nChannels = getNumberOfChannels(filePointer);
            nBlocks = getNumberOfBlocks(filePointer);
